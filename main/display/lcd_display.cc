...
LcdDisplay::LcdDisplay(esp_lcd_panel_io_handle_t panel_io, esp_lcd_panel_handle_t panel, int width, int height)
    ...
    std::string theme_name = "dark";//settings.GetString("theme", "light");
...
void LcdDisplay::SetEmotion(const char* emotion) {
 ...
 void LcdDisplay::setDisplayOnOff(bool on)
{
    if (panel_)
    {
        ...
        if (_backlight)
        {
            on?_backlight->RestoreBrightness():_backlight->SetBrightness(0);
        }
    }
}
