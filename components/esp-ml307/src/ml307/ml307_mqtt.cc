...
bool Ml307Mqtt::Connect(const std::string broker_address, int broker_port, const std::string client_id, const std::string username, const std::string password) {
    ...
    if (IsConnected()) {
        ...
        if (!(bits & MQTT_DISCONNECTED_EVENT)) {
            ...
            SetLastConnectError(MqttConnectError::Failed, "Failed to disconnect from previous MQTT connection");
            return false;
        }
    }
    ...
    if (!at_uart_->SendCommand(std::string("AT+MQTTCFG=\"keepalive\",") + std::to_string(mqtt_id_) + "," + std::to_string(keep_alive_seconds_))) {
        ...
        SetLastConnectError(MqttConnectError::Failed, "Failed to set MQTT keepalive interval");
        return false;
    }
    if (!at_uart_->SendCommand(std::string("AT+MQTTCFG=\"pingreq\",") + std::to_string(mqtt_id_) + "," + std::to_string(keep_alive_seconds_))) {
        ...
        SetLastConnectError(MqttConnectError::Failed, "Failed to set MQTT ping interval");
        return false;
    }
    ...
    if (!at_uart_->SendCommand(command)) {
        ...
        SetLastConnectError(MqttConnectError::Failed, "Failed to create MQTT connection");
        return false;
    }
    ...
    if (!(bits & MQTT_CONNECTED_EVENT)) {
        ...
        if (bits & MQTT_DISCONNECTED_EVENT) {
            SetLastConnectError(MqttConnectError::Rejected, "Disconnected: rejected or network error");
        } else {
