...
bool EspMqtt::Connect(const std::string broker_address, int broker_port, const std::string client_id, const std::string username, const std::string password) {
    ...
    if (!(bits & MQTT_CONNECTED_EVENT)) {
        if (bits == 0) {
            SetLastConnectError(MqttConnectError::Timeout, "MQTT connection timeout");
        } else if (last_connect_error_ == MqttConnectError::None) {
            SetLastConnectError(MqttConnectError::Failed, "MQTT connection failed");
        }
    }
...
void EspMqtt::MqttEventCallback(esp_event_base_t base, int32_t event_id, void *event_data) {
    ...
    case MQTT_EVENT_ERROR: {
        ...
        auto error = event->error_handle;
        const char* error_name = error ? esp_err_to_name(error->esp_tls_last_esp_err) : nullptr;
        std::string error_message = error_name ? error_name : "MQTT error";
        if (error != nullptr) {
            error_message += ", type=" + std::to_string(error->error_type);
            error_message += ", return_code=" + std::to_string(error->connect_return_code);
            error_message += ", sock_errno=" + std::to_string(error->esp_transport_sock_errno);
        }
        ESP_LOGI(TAG, "MQTT error occurred: %s", error_message.c_str());
        if (error != nullptr && error->error_type == MQTT_ERROR_TYPE_CONNECTION_REFUSED) {
            switch (error->connect_return_code) {
                case MQTT_CONNECTION_REFUSE_PROTOCOL:
                    SetLastConnectError(MqttConnectError::ProtocolRejected, error_message);
                    break;
                case MQTT_CONNECTION_REFUSE_ID_REJECTED:
                    SetLastConnectError(MqttConnectError::ClientIdRejected, error_message);
                    break;
                case MQTT_CONNECTION_REFUSE_BAD_USERNAME:
                case MQTT_CONNECTION_REFUSE_NOT_AUTHORIZED:
                    SetLastConnectError(MqttConnectError::AuthFailed, error_message);
                    break;
                default:
                    SetLastConnectError(MqttConnectError::Rejected, error_message);
                    break;
            }
        } else if (error != nullptr && error->esp_tls_last_esp_err == ESP_ERR_ESP_TLS_CANNOT_RESOLVE_HOSTNAME) {
            SetLastConnectError(MqttConnectError::DnsFailed, error_message);
        } else {
            SetLastConnectError(MqttConnectError::Failed, error_message);
        }
