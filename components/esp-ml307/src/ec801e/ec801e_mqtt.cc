...
bool Ec801EMqtt::Connect(const std::string broker_address, int broker_port, const std::string client_id, const std::string username, const std::string password) {
    SetLastConnectError(MqttConnectError::None);
    EventBits_t bits;
    ...
    if (broker_port == 8883) {
        ...
        if (!at_uart_->SendCommand(std::string("AT+QMTCFG=\"ssl\",") + std::to_string(mqtt_id_) + ",1,2")) {
            ...
            SetLastConnectError(MqttConnectError::Failed, "Failed to set MQTT to use SSL");
            return false;
        }
    }
    ...
    if (!at_uart_->SendCommand(std::string("AT+QMTCFG=\"version\",") + std::to_string(mqtt_id_) + ",4")) {
        ...
        SetLastConnectError(MqttConnectError::Failed, "Failed to set MQTT version to 3.1.1");
        return false;
    }
    ...
    if (!at_uart_->SendCommand(std::string("AT+QMTCFG=\"session\",") + std::to_string(mqtt_id_) + ",1")) {
        ...
        SetLastConnectError(MqttConnectError::Failed, "Failed to set MQTT clean session");
        return false;
    }
    ...
    if (!at_uart_->SendCommand(std::string("AT+QMTCFG=\"keepalive\",") + std::to_string(mqtt_id_) + "," + std::to_string(keep_alive_seconds_))) {
        ...
        SetLastConnectError(MqttConnectError::Failed, "Failed to set MQTT keep alive");
        return false;
    }
    ...
    if (!at_uart_->SendCommand("AT+QMTCFG=\"dataformat\"," + std::to_string(mqtt_id_) + ",0,1")) {
        ...
        SetLastConnectError(MqttConnectError::Failed, "Failed to set MQTT to use HEX encoding");
        return false;
    }
    ...
    if (!at_uart_->SendCommand(command)) {
        ...
        SetLastConnectError(MqttConnectError::Failed, "Failed to open MQTT connection");
        return false;
    }
    ...
    if (bits & EC801E_MQTT_OPEN_FAILED) {
        ...
        switch (error_code_) {
            case 4:
                SetLastConnectError(MqttConnectError::DnsFailed, message);
                break;
            case 5:
                SetLastConnectError(MqttConnectError::Rejected, message);
                break;
            default:
                SetLastConnectError(MqttConnectError::Failed, message);
                break;
        }

        if (error_code_ == 2) { // MQTT 标识符被占用
            SetLastConnectError(MqttConnectError::ClientIdRejected, message);
            ...
            if (!(bits & EC801E_MQTT_DISCONNECTED_EVENT)) {
                ...
                SetLastConnectError(MqttConnectError::Failed, "Failed to disconnect occupied MQTT identifier");
                return false;
            }
        ...
        SetLastConnectError(MqttConnectError::Timeout, "MQTT open timeout");
        return false;
    }
    ...
    if (!at_uart_->SendCommand(command)) {
        ...
        SetLastConnectError(MqttConnectError::Failed, "Failed to connect to MQTT broker");
        return false;
    }
    ...
    if (bits & EC801E_MQTT_DISCONNECTED_EVENT) {
        ...
        switch (error_code_) {
            case 1:
                SetLastConnectError(MqttConnectError::ProtocolRejected, message);
                break;
            case 2:
                SetLastConnectError(MqttConnectError::ClientIdRejected, message);
                break;
            case 4:
            case 5:
                SetLastConnectError(MqttConnectError::AuthFailed, message);
                break;
            default:
                SetLastConnectError(MqttConnectError::Rejected, message);
                break;
        ...
        ESP_LOGE(TAG, "MQTT connection timeout");
        SetLastConnectError(MqttConnectError::Timeout, "MQTT CONNACK timeout");
        return false;
    }
    return true;
}
